<template>
  <div>
    <div class="pagination">
      <!-- <h1>{{ startAndEnd }}</h1>这是测试的数据 -->
      <button @click="$emit('getPageNo', pageNo - 1)"
       :disabled="pageNo == 1">上一页</button>
      <button v-if="startAndEnd.start > 1" @click="$emit('gerPageNo', 1)"
        >1</button>
      <button v-if="startAndEnd.start > 2">.....</button>

      <!-- 中间连续页码的地方:v-for、数组、对象、数字、字符串 -->
      <button v-for="page in startAndEnd.end" :key="page" 
...
export default {
  ...
  // total:对应的是总数,pageSize对应的是每个页面的内容条数，pageNo表示当前的页面位置，continues表示应该有多少连续的页面，是个常数。
  computed: {
    //分页器一共多少页【总条数/每页展示条数】
    totalPage() {
      //向上取整数
    ...
    startAndEnd() {
      ...
      const { totalPage, continues, pageNo } = this;//问题：这里为什么要写一段这样的代码。
      //特殊情况:总共页数小于连续页码数
